...
export default function HomeMap({pictures, setShowMap}: HomeMapProps) {
  ...
  const [picture, setPictureToView] = useState(null)
  ...
  function handlePictureClose() {
    setPictureToView(null)
    trackPictureEvent('close')
  }

  function trackPictureEvent(action: string, name?: string) {
    const label = name || picture?.name

    trackEvent({action, category: 'picture', ...(label && {label})})
  }
